...
module.exports = {
    ...
    maj: async(duree, chan) => {
        if (duree > 214748364700) { //si dure trop long pour sleep/10
            duree = 214748364700; //prendre plus grande
        }
        stopmaj = false;
    ...
    stopmaj_f: async(chan) => {
        ...
        await sleep(1000);
        message = await message.edit(message.content.slice(0, -3) + "\n" + ">Alerte ! Corruption du système```");
        await sleep(1000);
        message = await message.edit(message.content.slice(0, -3) + "\n" + ">Redémarrage en mode sans echec....```");
        await sleep(10000);
        message = await message.edit(message.content.slice(0, -3) + "\n" + ">Signal redémarré avec succès !```");
    },

    mise_en_route: async(chan, version) => {
        let message = await chan.send("```Établissement de la liaison... 0%```");
        await sleep(1000);
