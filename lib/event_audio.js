...
async function play_audio(client, channel_log, file, volume, nb_chan, ID_chans = []) {
    ...
    dif_log("AudioEventsManager", `Diffusion de \`${file}\` sur le(s) caneaux/canal \`${get_corres_listeID_nom(chan_id_to_play).toString()}\``, channel_log, client.user.avatarURL(), "#8630F3");
