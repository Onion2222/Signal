    ...
    add(event, write_to_json = false) {
        ...
        if (event.CronPeriodText) { //si cron A FAIRE VALIDER CRON
            new_audioEvents.cron = new CronJob(event.CronPeriodText, () => {
                //console.log(event);
                play_audio(this.client, this.channel_log, event.filename, event.nbChan, event.IDChans);
            }, null, event.actif);
        }
...
async function play_audio(client, channel_log, file, nb_chan, ID_chans = []) {
    ...
    dif_log("AudioEventsManager", `Diffusion de \`${file}\` sur les canneaux \`${get_corres_listeID_nom(chan_id_to_play).toString()}\``, channel_log, client.user.avatarURL(), "#8630F3");
