...
class AudioEventsManager {
    ...
    stop(nom) { //a faire gestion erreur
        ...
        this.audioEvents.find(event => event.nom == nom).cron.stop();
        let liste_audioEvents = JSON.parse(fs.readFileSync(this.path_to_json, 'utf8')); //recup le json actuel
    ...
    start(nom) { //a faire gestion erreur
        ...
        this.audioEvents.find(event => event.nom == nom).cron.start();
    ...
    info(nom) {
        let liste_audioEvents = JSON.parse(fs.readFileSync(this.path_to_json, 'utf8'));
        let info_cron = this.audioEvents.find(event => event.nom == nom);
        let info_param = liste_audioEvents.Audio_events.find(event => event.nom == nom);
        if (!info_cron) return 1; //si ne trouve pas
        if (!info_param) return 2; //si ne trouve pas
        return [info_cron, info_param];
    }
    running(nom) {
        return this.audioEvents.find(event => event.nom == nom).cron.running;
    }
    lastDate(nom) {
        return this.audioEvents.find(event => event.nom == nom).cron.lastDate();
    }
    nextDates(nom) {
        return this.audioEvents.find(event => event.nom == nom).cron.nextDates();
    }
}
