...
module.exports = {
    ...
    dif_log: async(titre, log_txt, channel, avatar, couleur = "#000000", MP = false, fichier = false) => {
        ...
        log_txt = dateToStringReduit(now) + log_txt;
    ...
    get_usernames: (member, tag) => {
        let usernames = "";
        if (member.nickname) usernames += member.nickname + " | ";
        usernames += member.user.tag;
        if (tag) usernames += " (<@" + member.user.id + ">)";
        return usernames;
    },
    get_corres_ID_nom: (ID) => {
        return this.get_corres_ID_nom(ID);
    },
    get_corres_listeID_nom: (listeID) => {
        let liste_nom = [];
        for (let index = 0; index < listeID.length; index++) {
            liste_nom.push(get_corres_ID_nom(listeID[index]));
        }
        return liste_nom;
    }
...
function get_corres_ID_nom(ID) {
    let configuration = require("../data/conf_signal.json");
    return configuration.frequence.freq.find(frequence => frequence.ID == ID).nom;
}
