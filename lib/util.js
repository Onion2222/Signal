...
module.exports = {
    ...
    validator: (text, mots_interdits) => {
        ...
        for (let mot of mots_interdits) {
            let regex = new RegExp(mot[0], "i");
            //console.log(text.search(regex));
            if (text.search(regex) != -1) liste_mot_trouve.push(mot[1]);
        }
    ...
    brouiller: (text, niveau_espace, niveau_caractere, brouille_espace, brouille_caractere, caractere_autorise) => {
        let new_text = "";
        for (i = 0; i < text.length; i++) {
            if (text.charAt(i) === " " & ((new_text.length + (text.length - i)) < 1900)) { //si ESPACE   (bug car message discord limité à 2048 caract)
                ...
                if (randomTF(niveau_espace) & niveau_espace != 0) { //si 3 => output 0 1 2
                    ...
                    new_text += brouille_espace[random_priv(brouille_espace.length)];
                }
                new_text += " ";
            } else {
                
                if (niveau_caractere != 0 & caractere_autorise.indexOf(text.charAt(i)) == -1 & randomTF(niveau_caractere) ) {
                    /*
                    switch (random_priv(10)) { //pk pas ascii ?
                        case 0:
                            new_text += "%";
                            break;
                        case 1:
                    ...
                    new_text += brouille_caractere[random_priv(brouille_caractere.length)];
                } else {
